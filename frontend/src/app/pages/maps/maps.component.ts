...
export class MapsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Handler para resize del viewport (debounced)
  ...
  /**
   * Handler cuando el usuario selecciona una orden del select
   */
  onSelectChange(orderId: any): void {
    if (!orderId) return;
    const id = Number(orderId);
    const order = this.ordersWithTracking.find((o) => Number(o.id) === id);
    if (order) {
      // iniciar tracking para la orden seleccionada
      this.startTracking(order);
    }
  }

  /**
   * Refrescar mapa: invalidateSize y recentrar si hay marcadores
   */
  refreshMap(): void {
    try {
      this.map.invalidateSize();
      if (this.motorcycleMarkers.size > 0) {
        // centrar en el primer marcador
        const first = Array.from(this.motorcycleMarkers.values())[0];
        this.map.setView(first.positions[first.positions.length - 1]);
      }
    } catch (e) {
  ...
  ngAfterViewInit(): void {
    ...
    // Escuchar resize para ajustar el mapa dinámicamente
  ...
  private initMap(): void {
    ...
    // Asegurar que Leaflet recalcule el tamaño después de la renderización
    setTimeout(() => {
      try {
        this.map.invalidateSize();
      } catch (e) {
  ...
  private loadOrders(): void {
    this.ordenService.list().subscribe({
      next: (orders) => {
        this.ordersWithTracking = orders.filter(
          (o) => o.motorcycle_id && o.status !== "delivered"
        );
  ...
  private calculateStats(): void {
    this.ordenService.list().subscribe({
      next: (orders) => {
        ...
        this.completedToday = orders.filter(
  ...
  startTracking(order: any): void {
    ...
    this.motoService.view(order.motorcycle_id).subscribe({
      next: (motorcycle) => {
        ...
        this.http
          .post(
            `${environment.url_backend}/motorcycles/track/${licensePlate}`,
            {}
          )
          .subscribe({
            next: (response: any) => {
    ...
    // Forzar recalculo del tamaño tras centrar
    setTimeout(() => {
      try {
        this.map.invalidateSize();
      } catch (e) {}
  ...
  stopTracking(order: any): void {
    this.motoService.view(order.motorcycle_id).subscribe({
      next: (motorcycle) => {
        ...
        this.http
          .post(
            `${environment.url_backend}/motorcycles/stop/${licensePlate}`,
            {}
          )
          .subscribe({
            next: (response: any) => {
  ...
  stopAllTracking(): void {
    this.motorcycleMarkers.forEach((data, licensePlate) => {
      this.http
        .post(`${environment.url_backend}/motorcycles/stop/${licensePlate}`, {})
        .subscribe({
          next: () => {
   ...
   */
  private updateMotorcyclePosition(
    licensePlate: string,
    coordinates: any,
    orderId: number
  ): void {
    ...
    if (!motorcycleData) {
      ...
      const marker = L.marker(newLatLng, { icon: motorcycleIcon }).addTo(
        this.map
      ).bindPopup(`
